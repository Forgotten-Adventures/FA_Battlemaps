...
function cleanPackEntry(data, { clearSourceId = true, ownership = 0 } = {}) {
  ...
  Object.entries(data.flags).forEach(([key, contents]) => {
    ...
    } else if (Object.keys(contents).length === 0) delete data.flags[key];
  });

  // Remove mystery-man.svg from Actors
  if (["character", "container", "creature", "vehicle", "vehicle_npc"].includes(data.type) && data.img === "icons/svg/mystery-man.svg") {
    data.img = "";
  ...
  if (data.data?.description) data.data.description = cleanString(data.data.description);
